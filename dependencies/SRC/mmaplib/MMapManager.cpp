...
MMapManager::MMapManager(const char* dataPath, uint32 mapid) : m_dataPath(dataPath), ManagerMapId(mapid), m_navMesh(NULL), m_navMeshQuery(NULL), lastTileRef(0)
{
    ...
    snprintf(fileName, pathLen, (m_dataPath+"/%03i.mmap").c_str(), ManagerMapId);
...
float MMapManager::calcAngle( float Position1X, float Position1Y, float Position2X, float Position2Y )
{
    ...
    if (dx == 0.0)
    {
        if (dy == 0.0)
            ...
            angle = 3.14159265358979323846f * 0.5 /* / 2 */;
        else
            angle = 3.14159265358979323846f * 3.0 * 0.5/* / 2 */;
    }
    else if (dy == 0.0)
    {
        if (dx > 0.0)
    ...
    {
        if (dx < 0.0)
            angle = atanf(dy/dx) + 3.14159265358979323846f;
        else if (dy < 0.0)
            angle = atanf(dy/dx) + (2*3.14159265358979323846f);
        else
            angle = atanf(dy/dx);
    }

    // Convert to degrees
    angle = angle * float(180.f / 3.14159265358979323846f);
...
bool MMapManager::LoadNavMesh(uint32 x, uint32 y)
{
    ...
    if(itr == TileReferences.end())
    {
        ...
        else if(dtStatusFailed(dtresult))
        {
            free(data);
            return false;
        } else TileReferences.insert(std::make_pair(PackedTileID, new TileReferenceC(reference)));
...
Position MMapManager::getNextPositionOnPathToLocation(float startx, float starty, float startz, float endx, float endy, float endz)
{
    if(m_navMesh == NULL)
...
bool MMapManager::getNextPositionOnPathToLocation(float startx, float starty, float startz, float endx, float endy, float endz, Position& out)
{
    if(m_navMesh == NULL)
        return false;
...
Position MMapManager::getBestPositionOnPathToLocation(float startx, float starty, float startz, float endx, float endy, float endz)
{
    if(startx == endx && starty == endy && startz == endz)
        return Position(endx, endy, endz); // Fixed below but save us some time here.

    Position pos(startx, starty, startz);
    Position nextpos(startx, starty, startz);
    Position returnpos(endx, endy, endz);
...
PositionMapContainer* MMapManager::BuildFullPath(unsigned short moveFlags, float startx, float starty, float startz, float endx, float endy, float endz, bool straight)
{
    if(m_navMesh == NULL)
        return NULL;
    ...
    mPathFilter->setIncludeFlags(moveFlags);
    ...
    PositionMapContainer* map = new PositionMapContainer();
    uint32 counter = 0;
...
bool MMapManager::GetWalkingHeightInternal(float positionx, float positiony, float positionz, float endz, Position& out)
{
    if(m_navMesh == NULL)
        return false;
