...
static void addAndCarry(const g3d_uint64& _a, const g3d_uint64& _b, g3d_uint64& carry, g3d_uint64& result) {
    ...
    g3d_uint32 a [2] = {_a & 0xFFFFFFFF, _a >> 32};
    g3d_uint32 b [2] = {_b & 0xFFFFFFFF, _b >> 32};

    g3d_uint64 tmp = g3d_uint64(a[0]) + b[0];

    result = tmp & 0xFFFFFFFF;
    g3d_uint32 c = tmp >> 32;

    tmp = g3d_uint64(c) + a[1] + b[1];
    result += tmp << 32;
    carry = (tmp >> 32);
}

/** Multiplies two unsigned 64-bit integers, placing the result into one 64-bit int and the overflow into another.*/
void multiplyAndCarry(const g3d_uint64& _a, const g3d_uint64& _b, g3d_uint64& carry, g3d_uint64& result) {
...
uint128::uint128(const g3d_uint64& hi, const g3d_uint64& lo) : hi(hi), lo(lo) {
}

uint128::uint128(const g3d_uint64& lo) : hi(0), lo(lo) {
}

uint128& uint128::operator+=(const uint128& x) {

    G3D::g3d_uint64 carry;
    addAndCarry(lo, x.lo, carry, lo);

    // Adding the carry will change hi. Save the old hi bits in case this == x.
    const g3d_uint64 xHi = x.hi;
    hi += carry;
    hi += xHi;
    return *this;
}
...
uint128& uint128::operator*=(const uint128& x) {
    ...
    const g3d_uint64 oldLo = lo;
    const g3d_uint64 oldXLo = x.lo;
...
uint128& uint128::operator>>=(const int x) {
    ...
    g3d_uint64 mask = ((g3d_uint64)1L << x) - 1;
    g3d_uint64 tmp = hi & mask;
    hi >>= x;
...
uint128& uint128::operator<<=(const int x) {
    ...
    g3d_uint64 mask = ~((((g3d_uint64)1L) << (64 - x)) - 1);
    g3d_uint64 tmp = lo & mask;
    lo <<= x;
