...
bool ConvertADT(FILE *out_file, char *filename, int /*cell_y*/, int /*cell_x*/, uint32 build)
{
    ...
    fwrite(&heightHeader.gridMaxHeight, sizeof(float), 1, out_file);
    // Store height data
    if (!(heightHeader.flags & MAP_HEIGHT_NO_HEIGHT))
    {
    ...
    } else fwrite(&liquidHeader.liquidLevel, sizeof(float), 1, out_file);
...
void ExtractMapsFromMpq(uint32 build)
{
    ...
    for (uint32 z = 0; z < map_count; ++z)
    {
        WDT_file wdt;
        FILE *out_file = NULL;
        sprintf(output_filename, "%s/maps/Map_%03u.bin", output_path, map_ids[z].id);
