...
public:
    ...
    RONIN_INLINE void ResetLastFollowPos() { m_lastFollowX = m_lastFollowY = 0.0f; };

    RONIN_INLINE void SetFormationLinkTarget(Creature* cr)  { m_FormationLinkTarget = cr; };
    RONIN_INLINE Creature* GetFormationLinkTarget() { return m_FormationLinkTarget; }

    RONIN_INLINE uint32 GetFormationSQLId() { return m_FormationLinkSqlId; };
    RONIN_INLINE void SetFormationSQLId(uint32 Id) { m_FormationLinkSqlId = Id; }

    RONIN_INLINE float GetFormationFollowAngle() { return m_FormationFollowAngle; };
    RONIN_INLINE void SetFormationFollowAngle(float Angle) { m_FormationFollowAngle = Angle; }

    RONIN_INLINE float GetFormationFollowDistance() { return m_FormationFollowDistance; };
    RONIN_INLINE void SetFormationFollowDistance(float Dist) { m_FormationFollowDistance = Dist; }

    RONIN_INLINE float GetFollowDistance() { return FollowDistance; };
    RONIN_INLINE void SetFollowDistance(float dist) { FollowDistance = dist; };

    RONIN_INLINE float GetUnitToFollowAngle() { return m_FollowAngle; }
    RONIN_INLINE void SetUnitToFollowAngle(float angle) { m_FollowAngle = angle; }

    RONIN_INLINE void SetUnitToFear(Unit* un)  { UnitToFear = un; };
    RONIN_INLINE Unit* getUnitToFear() { return UnitToFear; }

    RONIN_INLINE void SetUnitToFollow(Unit* un) { UnitToFollow = un; };
    RONIN_INLINE Unit* getUnitToFollow() { return UnitToFollow; }

    RONIN_INLINE void SetBackupUnitToFollow(Unit* un) { UnitToFollow_backup = un; };
    ...
    RONIN_INLINE void setMoveRunFlag(bool f) { m_moveRun = f; }
    RONIN_INLINE bool getMoveRunFlag() { return m_moveRun; }

    RONIN_INLINE void setMoveFlyFlag(bool f) { m_moveFly = f; }
    RONIN_INLINE bool getMoveFlyFlag() { return m_moveFly; }

    RONIN_INLINE void setMoveSprintFlag(bool f) { m_moveSprint = f; }
    RONIN_INLINE bool getMoveSprintFlag() { return m_moveSprint; }

    RONIN_INLINE void setCanMove(bool c) { m_canMove = c; }
    RONIN_INLINE bool getCanMove() { return m_canMove; }

    RONIN_INLINE void setFearTimer(uint32 time) { m_FearTimer = time; }
    RONIN_INLINE uint32 getFearTimer() { return m_FearTimer; }
    ...
    RONIN_INLINE LocationVector GetDestPos()
    {
        if((!m_ignorePathMap && PathMap != NULL) || m_pathfinding)
    ...
    RONIN_INLINE LocationVector GetReturnPos() { return LocationVector(m_returnX, m_returnY, m_returnZ, m_returnO); }
    RONIN_INLINE LocationVector GetNextPos() { return LocationVector(m_nextPosX, m_nextPosY, m_nextPosZ, m_nextPosO); }

    float GetMovementTime(float distance);
    RONIN_INLINE void SetSourcePos(float x, float y, float z) { m_sourceX = x; m_sourceY = y; m_sourceZ = z; };
    RONIN_INLINE void SetReturnPos(float x, float y, float z, float o = 0.0f) { m_returnX = x; m_returnY = y; m_returnZ = z; m_returnO = o; };
    RONIN_INLINE void SetNextPos(float x, float y, float z, float o = 0.0f) { m_nextPosX = x; m_nextPosY = y; m_nextPosZ = z; m_nextPosO = o; };
    RONIN_INLINE void SetDestPos(float x, float y, float z, float o = 0.0f) { m_destinationX = x; m_destinationY = y; m_destinationZ = z; m_destinationO = o; };
