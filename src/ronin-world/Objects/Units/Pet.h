...
public:
    ...
    RONIN_INLINE void SetPetAction(uint32 act) { m_Action = act; }
    RONIN_INLINE uint32 GetPetAction(void) { return m_Action; }

    RONIN_INLINE void SetPetState(uint32 state) { m_State = state; }
    RONIN_INLINE uint32 GetPetState(void) { return m_State; }

    RONIN_INLINE void SetPetDiet(uint32 diet) { m_Diet = diet; }
    RONIN_INLINE void SetPetDiet() { m_Diet = myFamily ? myFamily->petDietFlags : 0; }

    RONIN_INLINE uint32 GetPetDiet(void) { return m_Diet; }
    ...
    RONIN_INLINE Player* GetPetOwner() { return m_Owner; }
    RONIN_INLINE void ClearPetOwner() { m_Owner = NULL; }
    ...
    RONIN_INLINE void RemoveSpell(uint32 SpellID)
    {
        SpellEntry *sp = dbcSpell.LookupEntry(SpellID);
        if(sp)
            RemoveSpell(sp);
    }
    RONIN_INLINE void SetSpellState(uint32 SpellID, uint16 State)
    {
        SpellEntry *sp = dbcSpell.LookupEntry(SpellID);
        if(sp) SetSpellState(sp, State);
    }
    RONIN_INLINE uint16 GetSpellState(uint32 SpellID)
    {
        if(SpellID == 0)
    ...
    RONIN_INLINE PetSpellMap* GetSpells() { return &mSpells; }
    RONIN_INLINE bool IsSummonedPet() { return Summon; }
    bool IsWarlockPet()
    {
    ...
    RONIN_INLINE std::string& GetPetName() { return m_name; }
    ...
    uint32 GetHappiness() { return GetPower(POWER_TYPE_HAPPINESS); };
    ...
    /* Pet Talents! */
    RONIN_INLINE uint8 GetUnspentPetTalentPoints() { return GetByte( UNIT_FIELD_BYTES_1, 1); }
    RONIN_INLINE void SetUnspentPetTalentPoints(uint8 points) { SetByte(UNIT_FIELD_BYTES_1, 1, points);}
    RONIN_INLINE uint8 GetSpentPetTalentPoints() { return GetPetTalentPointsAtLevel() - GetUnspentPetTalentPoints(); }
