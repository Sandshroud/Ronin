...
bool GMWarden::HandleAddInvItemCommand(const char *args, WorldSession *m_session)
{
    ...
    if(ItemPrototype* it = sItemMgr.LookupEntry(itemid))
    {
        ...
        if(uint32 leftover = chr->GetInventory()->AddItemById(itemid, count, randomprop, (ADDITEM_FLAG_CREATED|ADDITEM_FLAG_GIFTED), m_session->GetPlayer()))
        {
            m_session->SendNotification("Unable to add %u/%u of the items!", leftover, count);
            return true;
        }
...
bool GMWarden::HandleSummonCommand(const char* args, WorldSession *m_session)
{
    ...
    else
    {
        ...
        else
        {
            ...
            char query[512];
            snprintf((char*) &query,512, "UPDATE character_data SET mapId = '%u', instanceId = '%u', positionX = '%f', positionY = '%f', positionZ = '%f', zoneId = '%u', lastSaveTime = '%llu' WHERE guid = '%u';", pPlayer->GetMapId(), pPlayer->GetInstanceID(), pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), pPlayer->GetZoneId(), UNIXTIME, pinfo->charGuid.getLow());
