...
{
private:
    typedef RONIN_UNORDERED_MAP<uint32, LuaUnitBinding> UnitBindingMap;
    typedef RONIN_UNORDERED_MAP<uint32, LuaQuestBinding> QuestBindingMap;
    typedef RONIN_UNORDERED_MAP<uint32, LuaInstanceBinding> InstanceBindingMap;
    typedef RONIN_UNORDERED_MAP<uint32, LuaGameObjectBinding> GameObjectBindingMap;
    typedef RONIN_UNORDERED_MAP<uint32, LuaUnitGossipBinding> GossipUnitScriptsBindingMap;
    typedef RONIN_UNORDERED_MAP<uint32, LuaItemGossipBinding> GossipItemScriptsBindingMap;
    typedef RONIN_UNORDERED_MAP<uint32, LuaGOGossipBinding> GossipGOScriptsBindingMap;

    //maps to creature, & go script interfaces
    RONIN_MULTIMAP<uint32, LuaCreature*> m_cAIScripts;
    RONIN_MULTIMAP<uint32, LuaGameObject*> m_gAIScripts;
    RONIN_UNORDERED_MAP<uint32, LuaQuest*> m_qAIScripts;
    RONIN_UNORDERED_MAP<uint32, LuaInstance*> m_iAIScripts;
    RONIN_UNORDERED_MAP<uint32, LuaGossip*> m_unitgAIScripts;
    RONIN_UNORDERED_MAP<uint32, LuaGossip*> m_itemgAIScripts;
    RONIN_UNORDERED_MAP<uint32, LuaGossip*> m_gogAIScripts;

    RONIN_SET<int> m_pendingThreads, m_functionRefs;
    RONIN_MAP< uint64, RONIN_SET<int> > m_objectFunctionRefs;
...
public:
    ...
    HEARTHSTONE_INLINE RONIN_SET<int> & getThreadRefs() { return m_pendingThreads; }
    HEARTHSTONE_INLINE RONIN_SET<int> & getFunctionRefs() { return m_functionRefs; }
    ...
    HEARTHSTONE_INLINE RONIN_UNORDERED_MAP<uint32, LuaQuest*> & getLuQuestMap() { return m_qAIScripts; }
    HEARTHSTONE_INLINE RONIN_UNORDERED_MAP<uint32, LuaInstance*> & getLuInstanceMap() { return m_iAIScripts; }
    HEARTHSTONE_INLINE RONIN_UNORDERED_MAP<uint32, LuaGossip*> & getUnitGossipInterfaceMap() { return m_unitgAIScripts; }
    HEARTHSTONE_INLINE RONIN_UNORDERED_MAP<uint32, LuaGossip*> & getItemGossipInterfaceMap() { return m_itemgAIScripts; }
    HEARTHSTONE_INLINE RONIN_UNORDERED_MAP<uint32, LuaGossip*> & getGameObjectGossipInterfaceMap() { return m_gogAIScripts; }
    HEARTHSTONE_INLINE RONIN_MAP< uint64, RONIN_SET<int> > & getObjectFunctionRefs() { return m_objectFunctionRefs; }
    RONIN_UNORDERED_MAP<int, EventInfoHolder*> m_registeredTimedEvents;
    std::vector<uint16> EventAsToFuncName[NUM_SERVER_HOOKS];
    RONIN_MAP<uint32, uint16> m_luaDummySpells;
